  ...
  <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center">
    <motion.div
      ...
      <img
        ...
        className="rounded-lg shadow-lg object-contain w-1/3 md:w-2/3 max-h-32"
      />
    </motion.div>
    <div className={`text-center md:text-left w-full md:w-1/2 ${imageLeft ? "md:order-2" : "md:order-1"}`}>
      ...
      {!hideAuthor && (
        <p className="text-white/60 mt-4 text-[clamp(0.875rem,2vw,1rem)] md:text-right">{`- ${author}`}</p>
      )}
    </div>
  </div>
);
